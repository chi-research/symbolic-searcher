...
contract Getter is IGetter {
  ...
  function get() public {
    ...
    uint256 orig_balance = IWETH(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2).balanceOf(dest);
    pool.burn(dest);
    uint256 new_balance = IWETH(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2).balanceOf(dest);
    assert(new_balance <= orig_balance);
  }
}